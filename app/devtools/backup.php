...
background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='100' height='3'>
  <path d='M2 30 0 M5 0 v30 M8 0 v30' fill='none' stroke='black' />
</svg>");
        ...
        <div style="display: inline-block;">
          ...
          <select id="domain" name="domain" onchange="this.form.submit();">
            ...
            <?php foreach ($dirs as $dir) { ?>
              <option <?= isset($_GET['domain']) && $_GET['domain'] == basename($dir) ? 'selected' : '' ?>><?= basename($dir); ?>
              </option>
            <?php } ?>
          </select>
        </div>
              ...
              <table
                ...
                foreach ($files as $key => $file) {
                  ...
                  <tr>
                    <td>
                      <fieldset name="">
                        <input id="files-<?= $key; ?>" type="checkbox" name="folder[<?= $dir ?>][]" value="<?= basename($file) ?>"
                          <?= !is_file($b_file) ? 'checked=""' : (md5_file($b_file) == $hash ?: 'checked=""') ?> />&nbsp;<a
                          href="<?= '?client=' . $_GET['client'] . '&domain=' . $_GET['domain'] . '&file=' . $dir . '/' . basename($file) ?>"
                          style="<?= md5_file($b_file) == $hash ?: 'color: red;' ?>"
                          title="<?= !is_file($b_file) ? '&lt;NO FILE FOUND&gt;' : md5_file($file) ?>"><?= basename($file) ?></a>
                      </fieldset>
                    </td>
                    <td style="color: <?= !is_file($b_file) ? 'black' : (md5_file($b_file) == $hash ? 'green' : 'red') ?>;">
                      <?= !is_file($b_file) ? '&lt;NO FILE FOUND&gt;' : '' /*md5_file($file)*/ ?>
                    </td>
                  </tr>
                <?php } ?>
              </table>
