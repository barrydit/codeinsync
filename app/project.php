...
#app_project-container.selected {
display : block;
z-index : 1;
...
/* background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='100' height='3'><path d='M2 30 L0 M5 0 V30 M8 0 V30' fill='none' stroke='black' /></svg>"); */
...
background-repeat: no-repeat; /* not valid */
...
if (false) { ?></style><?php }
ob_start(); ?>
<div id="app_project-container"
  class="absolute <?= get_included_files()[0] == __FILE__ || (isset($_GET['project']) && !$_GET['project']) && !isset($_GET['path']) ? 'selected' : '' ?>"
  style="z-index: 1; width: 724px; background-color: rgba(255,255,255,0.8); padding: 10px;">
  <div
    style="position: relative; margin: 0 auto; width: 704px; height: 506px; border: 3px dashed #DD0000; background-color: #FBF7F1;">
    <div class="absolute ui-widget-header"
      ...
      <div style="display: inline;">
        <span style="background-color: white; color: #DD0000;">Project
        ...
        <span style="background-color: #0078D7; color: white;"><code class="text-sm"
            style="background-color: white; color: #0078D7;">$ <?= defined('PHP_EXEC') ? APP_BASE['projects'] . 'index.php' : null; ?></code></span>
      </div>

      <div style="display: inline; float: right; text-align: center; color: blue;"><code
          style="background-color: white; color: #0078D7;"><a style="cursor: pointer; font-size: 13px;" onclick="document.getElementById('app_project-container').style.display='none';">[X]</a></code>
      </div>
    </div>

    <div class=" ui-widget-content"
      style="position: relative; display: block; width: 698px; background-color: rgba(251,247,241); z-index: 2; height: 500px; overflow: scroll;">

      <div class="splitter">
        <div id="first">
          <iframe id="app_project-iframe" src="<?= basename(/*'?project=show'*/ 'project.php') ?>"
        ...
        <form id="app_project-saveForm" method="POST">
          <div id="separator" style="height: 25px; text-align: center;">
            ...
            <div style="display: inline; float: right; z-index: 1000;">
              ...
              <button type="submit"
                style="background-color: white; cursor: pointer; border: 1px solid #000;">&nbsp;&nbsp;Save&nbsp;&nbsp;</button>
          ...
          <div id="second">
            ...
            <div id="app_project_editor" class="editor"
              style="display: <?= isset($_GET['file']) && isset($_GET['path']) && is_file($_GET['path'] . $_GET['file']) ? 'block' : 'block'; ?>; width: 778px; height: 287px; z-index: 2;"><textarea name="contents" class="ace_text-input" autocomplete="off" autocorrect="off" autocapitalize="off"
                ...
                wrap="off" placeholder="hello you&#10;Second line&#10;Third line"><?= htmlsanitize(is_file(APP_BASE['projects'] . 'index.php') ? file_get_contents(APP_BASE['projects'] . 'index.php') : '') /*   APP_BASE['clients'] . $_GET['client'] . '/' . $_GET['domain'] . '/' .  */ ?></textarea>
            </div>
          </div>
        </form>
      </div>
...
ob_start(); ?>
  ...
  document.getElementById('app_project-saveForm').addEventListener('submit', function (event) {
    ...
    $.ajax({
      url: '<?= basename(__FILE__); ?>?path=<?= APP_BASE['projects'] ?>&app=project&file=index.php',
      type: 'POST',
