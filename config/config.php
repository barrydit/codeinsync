...
if (!defined('APP_ROOT')) {
  ...
  if (array_key_first($_GET) != 'path') {
    ...
    $domainPath = isset($_GET['domain']) && $_GET['domain'] !== ''
        ...
        : (!empty($_ENV['DEFAULT_DOMAIN']) ? 'clientele' . DIRECTORY_SEPARATOR . $_ENV['DEFAULT_CLIENT'] . DIRECTORY_SEPARATOR . (array_key_first($_GET) == 'path' ? '' : ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['cmd']) && in_array($_POST['cmd'], ['cd ../', 'chdir ../']) ? '' : $_GET['domain'] = $_ENV['DEFAULT_DOMAIN']) . DIRECTORY_SEPARATOR ) : '')/*''*/;
