...
if (basename(__DIR__) == 'config') {
  //defined('APP_SELF') or define('APP_SELF', get_included_files()[0] ?? __FILE__);  
  //define('APP_PATH', /* $_ENV['APP_BASE_PATH'] ?? This may need to be fixed 1 day. */ ((basename(dirname(APP_SELF)) == 'public' ? dirname(APP_SELF, 2) : dirname(APP_SELF) ) ?? $_SERVER['DOCUMENT_ROOT']) . DIRECTORY_SEPARATOR); // dirname(__DIR__) . DIRECTORY_SEPARATOR
  //chdir('../');
