...
if (!is_dir(NODE_MODULES_PATH)) {
  ...
  $proc = proc_open(
    (stripos(PHP_OS, 'WIN') === 0 ? '' : APP_SUDO) . NPM_EXEC . ' install -g npm',
    [
      ["pipe", "r"],
      ["pipe", "w"],
      ["pipe", "w"]
  ...
  if (stripos(PHP_OS, 'WIN') !== 0) {
    ...
    if (!isset($errors['NPM-WEBPACK']) && !empty($errors['NPM-WEBPACK'])) {
      ...
      if (!is_dir(NODE_MODULES_PATH . 'webpack') || !is_dir(NODE_MODULES_PATH . 'webpack-cli')) {
        $proc = proc_open(
          (stripos(PHP_OS, 'WIN') === 0 ? '' : APP_SUDO) . NPM_EXEC . ' install webpack webpack-cli --save-dev',
          [
            ["pipe", "r"],
            ["pipe", "w"],
            ["pipe", "w"]
...
END
        ...
        if (!is_dir(NODE_MODULES_PATH . 'babel-loader')) {
          $proc = proc_open(
            (stripos(PHP_OS, 'WIN') === 0 ? '' : APP_SUDO) . NPM_EXEC . ' install babel-loader @babel/core @babel/preset-env --save-dev',
            [
              ["pipe", "r"],
              ["pipe", "w"],
              ["pipe", "w"]
...
END
        ...
        if (!is_file(APP_PATH . 'dist/bundle.js')) {
          $proc = proc_open(
            (stripos(PHP_OS, 'WIN') === 0 ? '' : APP_SUDO) . NPM_EXEC . ' run build',
            [
              ["pipe", "r"],
              ["pipe", "w"],
              ["pipe", "w"]
