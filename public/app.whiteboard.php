...
<div id="app_whiteboard-container"
  ...
  <div class="header ui-widget-header">
    ...
    <div style="display: inline; float: right; text-align: center;">[<a style="cursor: pointer; font-size: 13px;"
        onclick="document.getElementById('app_whiteboard-container').style.display='none';">X</a>]</div>
  </div>
...
ob_start(); ?>
  ...
  function finishDrawing(mouseEvent, element, context) {
    ...
    console.log(postArr);
    //data += '{[';
    //$.each(postArr, function(key, pos) { data += pos + (key < postArr.length - 1 ? "," : false); }); //data +=']}' ;
    $.ajax({
      type: "POST", url: 'http://localhost/app.whiteboard.php', contentType: "application/json",
      dataType: 'json', data: JSON.stringify(postArr), success: function () { console.log('test'); }, cache: false
    });
    postArr = new Array(); drawLine(mouseEvent, element, context); context.closePath(); // unbind any events which could draw
