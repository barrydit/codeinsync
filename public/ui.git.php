...
if ($path = realpath(APP_PATH . APP_BASE['config'] . 'git.php'))
  ...
  die(var_dump("$path path was not found. file=git.php"));
...
<div id="app_git-container"
  ...
  <div
    ...
    <div class="absolute" style="position: absolute; bottom: 24px; left: 0; right: 0; width: 100%; text-align: center;">
      <span style="text-decoration-line: underline; text-decoration-style: solid;">Git is a distributed version control
        system</span>
    </div>
      ...
      <div id="app_git-frameMenu" class="app_git-frame-container selected absolute"
        ...
        <div style="display: block; margin: 10px auto; width: 100%; background-color: rgb(255,255,255,.75);">
          ...
          <div id="app_git-commit-msg"
            style="position: absolute; display: none; top: 30px; left: 20%; right: 50%; border: 1px solid #000;">
            <div style="position: absolute; top: -20px; left: -20px; color: red; font-weight: bold;">
              <a style="cursor: pointer; font-size: 13px;"
                onclick="document.getElementById('app_git-commit-msg').style.display='none';">[X]</a>
            </div>
          ...
          <div id="app_git-oauth"
            ...
            <div
              ...
              <a style="cursor: pointer; font-size: 13px;"
                onclick="document.getElementById('app_git-oauth').style.display='none';">[X]</a>
            </div>
            <form action="" method="POST">
              <input id="app_git-oauth-input" type="text" placeholder="ghp_54nj8sA53dZi9jgf..." size="26"><button
                type="submit" style="background-color: #FFF;"> Save </button><br />
            </form>
            <div
              style="position: absolute; bottom: -20px; right: 0px; color: red; font-weight: bold; background: #FFF;">
              <a href="https://github.com/settings/tokens">Github.com-&gt;settings-&gt;tokens</a>
            </div>

          </div>

          <div id="app_git-clone-url"
            style="position: absolute; display: none; top: 80px; left: 20%; right: 50%; border: 1px solid #000;">
            <div style="position: absolute; top: 8px; left: 60px; color: red; font-weight: bold;">
              <a style="cursor: pointer; font-size: 13px;"
                onclick="document.getElementById('app_git-clone-url').style.display='none';">[X]</a>
            </div>
            <input id="app_git-clone-url-input" type="text" placeholder="https://github.com/barrydit/....git"
              size="26" />
          </div>
          ...
          <div
            ...
            <select id="app_git-frameSelector">
              ...
              <?php if (isset($errors['GIT_UPDATE'])) { ?>
                <option value="pull">pull</option><?php } ?>
...
ob_start(); ?>
  ...
  for (var i = 0; i < appGitPushElements.length; i++) {
    appGitPushElements[i].addEventListener('click', function () {
      ...
      document.getElementById('requestInput').value = 'git push https://<?= getenv('COMPOSER_TOKEN') ?>@github.com/barrydit/<?= $_GET['domain'] ?? ($_GET['project'] ?? 'codeinsync.git') ?>';
  ...
  document.getElementById('app_git-cmd-selected').addEventListener('submit',
    function (event) { // /* Prevent the default form submission */ event.preventDefault(); var
      ...
      var requestSubmit = document.getElementById('requestSubmit'); // Create a new click event
      var clickEvent = new MouseEvent('click', { bubbles: true, cancelable: true, view: window });
      // Dispatch the click event on the element
      requestSubmit.dispatchEvent(clickEvent); // var changePositionBtn = document.getElementById('changePositionBtn'); // const myDiv=document.getElementById('myDiv'); //
      if (myDiv.style.position == 'absolute') // changePositionBtn.dispatchEvent(clickEvent); isFixed=true; show_console();
        // For example, you can show an alert to indicate that the form submission is disabled
        console.log(cmdSelect.value + ' was executed.');
    });
  $(document).ready(function () {
    var git_frame_containers = $(".app_git-frame-container"); var
      git_frame_totalFrames = git_frame_containers.length; var currentIndex = 0; $("#app_git-frameSelector").value = 0;
    console.log(git_frame_totalFrames + ' - total frames'); /* $("#appGitAuthLabel").click(function() { if
    ($('#appGitAuthJsonForm').css('display')=='none' ) { $('#appGitAuthLabel').html("&#9650; <code>GIT_HOME/auth.json");
    ...
    */
    ...
    $("#app_git-frameMenuPrev").click(function () {
      if (currentIndex <= 0) currentIndex = 5; console.log(currentIndex + '!=' + git_frame_totalFrames); currentIndex--; if
        (currentIndex >= git_frame_totalFrames) {
        currentIndex = 0;
      }
      if (currentIndex == 0) {
