...
<div id="app_ace_editor-container"
  style="display: <?= (isset($_GET['app']) && $_GET['app'] == 'ace_editor' ? 'block' : (isset($_GET['file']) ? 'block' : 'none')) ?>; width: auto; border: 1px solid #000;">
  ...
  <div class="header ui-widget-header" style="margin: 10px;">
    ...
    <div style="display: inline; float: right; text-align: center;">[<a style="cursor: pointer; font-size: 13px;"
        onclick="document.getElementById('app_ace_editor-container').style.display='none';">X</a>]</div>
  </div>
  ...
  <div style="position: relative; display: inline-block; width: auto; padding-left: 10px;">
    <form style="display: inline;" autocomplete="off" spellcheck="false"
      action="<?= APP_URL_BASE . /*basename(__FILE__) .*/ '?' . http_build_query(APP_QUERY /*+ ['app' => 'ace_editor']*/) . (APP_ENV == 'development' ? '#!' : '') /* $c_or_p . '=' . (empty($_GET[$c_or_p]) ? '' : $$c_or_p->name) . '&amp;app=composer' */ ?>"
      method="GET">
  ...
  <form style="position: relative; display: inline;"
    action="<?= APP_URL_BASE . '?' . http_build_query(APP_QUERY + ['app' => 'ace_editor']) . (APP_ENV == 'development' ? '#!' : '') /* $c_or_p . '=' . (empty($_GET[$c_or_p]) ? '' : $$c_or_p->name) . '&amp;app=composer' */ ?>"
    method="POST">
    ...
    <div style="display: inline-block; width: auto; text-align: right; float: right;">
      <input type="submit" value="Save" class="btn" style="margin: -5px 5px 5px 0;"
...
if (cis_is_direct_http_file(__FILE__) && APP_DEBUG && ($_GET['app'] ?? null) === 'ace_editor')
...
<div id="app_composer-container"
  class="absolute <?= (APP_SELF == __FILE__ || (isset($_GET['app']) && $_GET['app'] == 'composer') && !isset($_GET['path']) || (defined('COMPOSER') && !is_object(COMPOSER)) || count((array) COMPOSER) === 0 || version_compare(COMPOSER_LATEST, COMPOSER_VERSION, '>') != 0 ? 'selected' : '') ?>"
  ...
  <div
    ...
    <div class="absolute ui-widget-header" id=""
      ...
      <div style="display: inline; padding-left: 40px;">
        <span style="background-color: #B0B0B0; color: white;">Composer
          <?= (version_compare(COMPOSER_LATEST, COMPOSER_VERSION, '>') != 0 ? 'v' . substr(COMPOSER_LATEST, 0, similar_text(COMPOSER_LATEST, COMPOSER_VERSION)) . '<span class="update" style="color: green; cursor: pointer;">' . substr(COMPOSER_LATEST, similar_text(COMPOSER_LATEST, COMPOSER_VERSION)) . '</span>' : 'v' . COMPOSER_VERSION); ?></span>
        <span>

          <form style="display: inline;" autocomplete="off" spellcheck="false"
            action="<?= APP_URL_BASE . '?' . http_build_query(APP_QUERY + ['app' => 'composer']) . (APP_ENV == 'development' ? '#!' : '') /* $c_or_p . '=' . (empty($_GET[$c_or_p]) ? '' : $$c_or_p->name) . '&amp;app=composer' */ ?>"
            method="GET">
    ...
    <div class="ui-widget-content"
      ...
      <div style="display: inline-block; text-align: left; width: 130px;">
        <div class="composer-menu text-sm"
          style="cursor: pointer; font-weight: bold; padding-left: 40px; border: 1px solid #000;">Main Menu</div>
      ...
      <div class="absolute"
        style="position: absolute; display: inline-block; top: 4px; text-align: right; width: 264px; ">
        <div class="text-xs" style="display: inline-block;">
          ...
          <a style="color: blue; text-decoration-line: underline; text-decoration-style: solid;"
            href="http://getcomposer.org/">http://getcomposer.org/</a>
        </div>
        <!--
...
-->
    ...
    <div class="absolute text-sm" style="position: absolute; bottom: 0; right: 0; padding: 2px; z-index: 1; ">
      <?= (version_compare(COMPOSER_LATEST, COMPOSER_VERSION, '>') != 0 ? '<code>Latest: </code><span class="update" style="color: green; cursor: pointer;">' . 'v' . substr(COMPOSER_LATEST, 0, similar_text(COMPOSER_LATEST, COMPOSER_VERSION)) . substr(COMPOSER_LATEST, similar_text(COMPOSER_LATEST, COMPOSER_VERSION)) . '</span>' : 'Installed: v' . COMPOSER_VERSION); ?>
    </div>
        ...
        foreach (COMPOSER->require as $key => $require) {
          if (preg_match('/.*\/.*:.*/', $key . ':' . $require))
            if (preg_match('/(.*)\/.*/', $key, $match))
              if (!empty($match) && !is_dir('vendor/' . $match[1] . '/'))
                $count++;
        }
      ?>
      ...
      <div id="app_composer-frameUpdate" class="app_composer-frame-container absolute"
        ...
        <form autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false"
          ...
          <div style="display: inline-block; width: 100%; margin: 0 auto;">
            ...
            <span style="background-color: white;">
              <span class="text-sm" style="display: inline-block;">was
                <?= (version_compare(COMPOSER_LATEST, COMPOSER_VERSION, '>') == 0 ? '<span style="font-weight: bold;">not</span>' : '') ?>
                found: </span>
            </span>
          </div>
          ...
          <div id="composerSetupForm"
            ...
            <div>
              <span class="text-xs"
                style="background-color: #0078D7; color: white;"><code>Version: (Installed) <?= COMPOSER_VERSION ?> -> (Latest) <?= COMPOSER_LATEST ?></code></span>
            </div>
...
sudo composer self-update</textarea>
      ...
      <div id="app_composer-frameInit"
        class="app_composer-frame-container absolute <?= realpath(COMPOSER_JSON['path']) ? '' : (defined('COMPOSER') && is_object(COMPOSER) && count((array) COMPOSER) !== 0 ? '' : 'selected'); ?>"
      ...
      <div id="app_composer-frameConf"
        class="app_composer-frame-container absolute <?= !defined('COMPOSER') && is_file(APP_PATH . 'composer.json') ? 'selected' : ''; ?>"
        ...
        <form autocomplete="off" spellcheck="false"
          ...
          <div id="appComposerAuthJsonForm"
            ...
            <span class="text-sm"
              style="float: right;"><?= ceil(abs((strtotime(date('Y-m-d')) - strtotime(date('Y-m-d', strtotime('+30 days', filemtime(COMPOSER_AUTH['path']))))) / 86400)) ?>
              (Days left)</span>
          ...
          <div
            ...
            <div class="text-sm" style="display: inline;">
              ...
              <label for="composerJson" id="appComposerConfigLabel"
                title="<?= (defined('COMPOSER_CONFIG') && realpath(COMPOSER_CONFIG['path']) ? COMPOSER_CONFIG['path'] : COMPOSER_CONFIG['path']) /*NULL*/ ; ?>"
                style="background-color: #6B4329; <?= (defined('COMPOSER_CONFIG') && realpath(COMPOSER_CONFIG['path']) ? 'color: #F0E0C6; text-decoration: underline; ' : 'color: red; text-decoration: underline; text-decoration: line-through;') ?> cursor: pointer; font-weight: bold;">&#9660;
          ...
          <div id="appComposerJsonForm"
            ...
            <?php if (defined('COMPOSER_JSON') && realpath(COMPOSER_JSON['path'])) { ?>
              <div style="display: inline-block; width: 100%; margin-bottom: 10px;">
                <div class="text-xs" style="display: inline-block; float: left; background-color: #0078D7; color: white;">
                  Last Update: <span <?= (isset(COMPOSER->time) && COMPOSER->time === '' ? 'style="background-color: white; color: red;"' : 'style="background-color: white; color: #0078D7;"') ?>><?= (isset(COMPOSER->time) && COMPOSER->time !== '' ? COMPOSER->{'time'} : date('Y-m-d H:i:s')) ?></span>
                </div>
            ...
            <div style="display: inline-block; width: 100%;"><span <?= (isset(COMPOSER->{'name'}) && COMPOSER->{'name'} !== '' ? '' : 'style="background-color: #fff; color: red;" title="Either Vendor or Package is missing"') ?>>Name:</span>
              <div style="position: relative; float: right;">
                ...
                <input type="text" id="tst" name="composer[config][name][vendor]" placeholder="vendor"
                  value="<?= (defined('COMPOSER') && isset(COMPOSER->name) ? explode('/', COMPOSER->name)[0] : ''); ?>"
                  size="13"> / <div class="absolute font-bold"
                  ...
                  value="<?= (defined('COMPOSER') && isset(COMPOSER->name) ? explode('/', COMPOSER->name)[1] : ''); ?>"
                  size="13" />
              </div>
            </div>
            <div style="display: inline-block; width: 100%;"><label for="composer-description"
                <?= (isset(COMPOSER->{'description'}) && COMPOSER->{'description'} !== '' ? '' : 'style="background-color: #fff; color: red; cursor: pointer;" title="Description is missing"') ?>>Description:</label>
              <div style="float: right;">
                <input id="composer-description" type="text" name="composer[config][description]" placeholder="Details"
                  value="<?= (defined('COMPOSER') && isset(COMPOSER->description) ? COMPOSER->description : ''); ?>">
              </div>
            </div>

            <!-- version -->
            <div style="display: inline-block; width: 100%;"><label for="composer-version"
                <?= (isset(COMPOSER->{'version'}) && preg_match(COMPOSER_EXPR_VER, COMPOSER->{'version'}) ? '' : 'style="background-color: #fff; color: red; cursor: pointer;" title="Version must follow this format: ' . COMPOSER_EXPR_VER . '"') ?>>Version:</label>
              <div style="float: right;">
                <input id="composer-version" type="text" name="composer[config][version]" size="10"
                  ...
                  value="<?= (defined('COMPOSER') && isset(COMPOSER->version) ? COMPOSER->version : ''); ?>">
              </div>
            </div>
            <!-- type -->
            <div style="display: inline-block; width: 100%;">Type:
              <div style="float: right;">
                <select name="composer[config][type]">
                  <option label="" <?= (defined('COMPOSER') && isset(COMPOSER->license) ? '' : 'selected=""'); ?>>
                  </option>
                  <?php foreach (['library', 'project', 'metapackage', 'composer-plugin'] as $type) { ?>
            ...
            <div style="display: inline-block; width: 100%;">Keywords:
              ...
              <div class="clearfix"></div>
              <div id="composerAppendKeyword"
                style="padding: 10px 0 10px 0; display: <?= (defined('COMPOSER') && isset(COMPOSER->keywords) && !empty(COMPOSER->keywords) ? 'block' : 'none') ?>; width: 100%;">
                <?php if (defined('COMPOSER') && isset(COMPOSER->keywords))
                  foreach (COMPOSER->keywords as $key => $keyword) { ?>
                    <label for="keyword_<?= $key; ?>"><sup
            ...
            <div style="display: inline-block; width: 100%;">Authors:<br />
              ...
              <?php if (defined('COMPOSER') && isset(COMPOSER->authors))
                foreach (COMPOSER->authors as $key => $author) { ?>
                  <div style="position: relative; float: left;">
                    <div class="absolute font-bold" style="position: absolute; top: -8px; left: 10px; font-size: 10px;">Name
                    </div>
...
-->
            ...
            <div style="display: inline-block; width: 100%;">
              ...
              <div id="composerAppendRequire"
                style="padding: 10px; display: <?= (defined('COMPOSER') && !isset(COMPOSER->{'require'}) ? 'none' : 'block') ?>;">
            ...
            <div style="display: inline-block; width: 100%;">Require-dev:
              ...
              <div id="composerAppendRequire-dev"
                style="padding: 10px; display: <?= (defined('COMPOSER') && !isset(COMPOSER->{'require-dev'}) ? 'none' : 'block') ?>;">
                <datalist id="composerReq-devVersResults">
                  <option value=""></option>
                </datalist>
                <?php $i = 0;
                if (defined('COMPOSER') && isset(COMPOSER->{'require-dev'}))
                  foreach (COMPOSER->{'require-dev'} as $key => $require) { ?>
                    <input type="checkbox" checked=""
      ...
      <?php if (version_compare(COMPOSER_LATEST, COMPOSER_VERSION, '>') == 0)
...
for (i = 0; i < dropdowns.length; i++) { var openDropdown=dropdowns[i]; if (openDropdown.classList.contains('show')) {
  openDropdown.classList.remove('show'); } } } } function rm_keyword(argv_id) { var
  el=document.querySelector('label[for=' + argv_id + ' ]'); var input=document.getElementById(argv_id); if (el)
  el.remove(); if (input) input.remove();
  //console.log(document.getElementById('composerAppendKeyword').childNodes.length); if
  (document.getElementById('composerAppendKeyword').childNodes.length==4)
  document.getElementById('composerAppendKeyword').style.display="none" ; } function add_keyword() { if
  (event.target.value=='' ) return; var
  filledInputs=$('#composerAppendKeyword').find(':input[type=text]').filter(function() {return !!this.value;}).length;
  document.getElementById('composerAppendKeyword').style.display="inline-block" ;
  keywordOption='<label class="text-sm" for="keyword_' + keyword_i + '" ><sup onclick="rm_keyword(\' keyword_' +
  keyword_i + '\' );">[x]</sup>' + event.target.value + '</label><input type="hidden" id="keyword_' + keyword_i + '"
    name="composer[config][keywords][]" value="' + event.target.value + '" />&nbsp;';
  keyword_i++;
  document.getElementById('composerAppendKeyword').insertAdjacentHTML('beforeend', keywordOption); // innerHTML +=
  keywordOption
  ...
  var filledInputs = $('#composerAppendRequire').find(':input[type=text]').filter(function() {return
  !!this.value;}).length;
  document.getElementById('composerAppendRequire').style.display = "inline-block";
  packageOption = '<input type="checkbox" checked
    onchange="this.indeterminate = !this.checked; document.getElementById(\'pkg_' + filledInputs + '\').disabled = !this.checked" />
  <input type="text" id="pkg_' + filledInputs + '" name="composer[config][require][]" value="' + event.target.value + '"
    list="composerReqVersResults" size="30" onSelect="get_version(\'pkg_' + filledInputs + '\')" /><label
    for="pkg_' + filledInputs + '"></label><br />';
  document.getElementById('composerAppendRequire').insertAdjacentHTML('beforeend', packageOption); // innerHTML +=
  packageOption
  event.target.value = "";
  }
  });
  ...
  */
  ...
  if(event.inputType == "insertReplacementText" || event.inputType == null) {
  var filledInputs = $('#composerAppendRequire-dev').find(':input[type=text]').filter(function() {return
  !!this.value;}).length;
  document.getElementById('composerAppendRequire-dev').style.display = "inline-block";
  packageOption = '<input type="checkbox" checked
    onchange="this.indeterminate = !this.checked; document.getElementById(\'pkg-dev_' + filledInputs + '\').disabled = !this.checked" /><input
    type="text" id="pkg-dev_' + filledInputs + '" name="composerRequireDevPkgs[]" value="' + event.target.value + '"
    list="composerReq-devVersResults" size="30" onSelect="get_dev_version(\'pkg-dev_' + filledInputs + '\')" /><label
    for="pkg-dev_' + filledInputs + '"></label><br />';
  document.getElementById('composerAppendRequire-dev').insertAdjacentHTML('beforeend', packageOption); // innerHTML +=
  packageOption
  event.target.value = "";
  }
  });
  ...
  */
  ...
  if (currentIndex <= 0) currentIndex=5; console.log(currentIndex + '!=' + totalFrames); currentIndex--; if
    ...
    composer_frame_containers.eq(currentIndex).addClass(' selected'); // css("z-index", totalFrames); // Set top layer
      z-index }); $("#frameSelector").change(function() { var selectedIndex=parseInt($(this).val(), 10);
      currentIndex=selectedIndex; $(".app_composer-frame-container").removeClass("selected"); // Remove selected class
      from all containers $(".app_composer-frame-container").eq(currentIndex).addClass("selected"); // Apply selected
      class to the chosen container }); /* $('select').on('change', function (e) { var
      ...
      */
      .btn {
      @apply rounded-md px-2 py-1 text-center font-medium text-slate-900 shadow-sm ring-1 ring-slate-900/10
      hover:bg-slate-50
      }
      ...
      <div id="app_git-container"
        class="absolute <?= (APP_SELF == __FILE__ || isset($_GET['app']) && $_GET['app'] == 'git' ? 'selected' : (version_compare(GIT_LATEST, GIT_VERSION, '>') != 0 ? (isset($_GET['app']) && $_GET['app'] != 'git' ? '' : '') : '')) ?>"
        style="z-index: 1; width: 424px; background-color: rgba(255,255,255,0.8); padding: 10px;">
        <div
          ...
          <div class="absolute ui-widget-header"
            ...
            <div style="display: inline; padding-left: 40px;">
              <span style="background-color: white; color: #F05033;">Git
                <?= (version_compare(GIT_LATEST, GIT_VERSION, '>') != 0 ? 'v' . substr(GIT_LATEST, 0, similar_text(GIT_LATEST, GIT_VERSION)) . '<span class="update" style="color: green; cursor: pointer;">' . substr(GIT_LATEST, similar_text(GIT_LATEST, GIT_VERSION)) . '</span>' : 'v' . GIT_VERSION); ?></span>
          ...
          <div class="absolute"
            ...
            <span style="text-decoration-line: underline; text-decoration-style: solid;">Git is a distributed version
              control system</span>
          </div>
          <div style="position: absolute; bottom: 0; left: 0; padding: 2px; z-index: 1;">
          ...
          <div style="position: relative; overflow: hidden; width: 398px; height: 286px;">
            ...
            <div id="app_git-frameMenu" class="app_git-frame-container selected absolute"
              ...
              <div style="position: relative; height: 100px;">
                <div id="app_git-commit_msg-container"
                  style="display: none; position: absolute; top: 80px; left: 25%; right: 25%;">
            ...
            <div id="app_git-frameInit" class="app_git-frame-container absolute"
              style="overflow: hidden; height: 270px;">
              <form autocomplete="off" spellcheck="false"
                action="?<?= http_build_query(APP_QUERY + ['app' => 'git']) . (APP_ENV == 'development' ? '#!' : '') /* $c_or_p . '=' . (empty($_GET[$c_or_p]) ? '' : $$c_or_p->name) . '&amp;app=git' */ ?>"
                method="POST">
                <div style="position: absolute; right: 0; float: right; text-align: center;">
            ...
            <div id="app_git-frameStatus" class="app_git-frame-container absolute"
              style="overflow: hidden; height: 270px;">
              <form autocomplete="off" spellcheck="false"
                action="?<?= http_build_query(APP_QUERY + ['app' => 'git']) . (APP_ENV == 'development' ? '#!' : '') /* $c_or_p . '=' . (empty($_GET[$c_or_p]) ? '' : $$c_or_p->name) . '&amp;app=git' */ ?>"
                method="POST">
                ...
                <div id="gitStatusForm"
                  ...
                  <textarea cols="40" rows="6"
                    name="git[status]"><?= /* GIT_INIT_PARAMS  NULL*/ 'git status'; ?></textarea>
                </div>
              </form>
            </div>

            <div id="app_git-frameConfig" class="app_git-frame-container absolute"
              style="overflow: hidden; height: 270px;">
              <form autocomplete="off" spellcheck="false"
                action="?<?= http_build_query(APP_QUERY + ['app' => 'git']) . (APP_ENV == 'development' ? '#!' : '') /* $c_or_p . '=' . (empty($_GET[$c_or_p]) ? '' : $$c_or_p->name) . '&amp;app=git' */ ?>"
                method="POST">
                <div style="position: absolute; right: 0; float: right; text-align: center;">
                  <button id="gitConfigSubmit" class="btn absolute"
                    style="position: absolute; top: 0; right: 0; z-index: 2;" type="submit" value>Modify</button>
                </div>
                <div style="display: inline-block; width: 100%; background-color: rgb(225,196,151,.75);">
                  <div class="text-sm" style="display: inline;">
                    <label id="gitStatusLabel" for="gitStatus"
                      style="background-color: hsl(29, 100%, 42%); color: white; cursor: pointer;">&#9650;
                      <code>GIT_PATH/.gitignore</code></label>
                  </div>
                </div>
                ...
                <div style="display: inline-block; width: 100%; background-color: rgb(225,196,151,.75);">
                  <div class="text-sm" style="display: inline;">
                    <label id="gitStatusLabel" for="gitStatus"
                      style="background-color: hsl(29, 100%, 42%); color: white; cursor: pointer;">&#9650;
                      <code>GIT_PATH/.gitconfig</code></label>
                  </div>
                </div>
                <div id="appGitConfigForm"
                  ...
                  <div style="display: none; width: 100%;">
                    ...
                    <div style="float: right;">
                      <input type="checkbox" name="gitIngoreFile" 1 /> <label
                        style="font-style:italic;">.gitignore</label>
                      <input type="checkbox" name="gitConfigFile" 1 /> <label
                        style="font-style:italic;">.gitconfig</label>
                    </div>
                  </div>
                  <div style="display: inline-block; width: 100%;">
                    <label>Name:</label>
                  ...
                  <div style="display: inline-block; width: 100%;">
                    ...
                    <div style="float: right;">
                      <input name="gitConfigCoreEditor" size="40"
                        value="\&quot;C:/Program Files (x86)/Programmer's Notepad/pn.exe\&quot; --allowmulti -w" />
                    </div>
                  </div>
                  <div style="display: inline-block; width: 100%;">
            ...
            <div id="app_git-frameCommit" class="app_git-frame-container absolute"
              style="overflow: hidden; height: 270px;">
              <form autocomplete="off" spellcheck="false"
                action="?<?= http_build_query(APP_QUERY + ['app' => 'git']) . (APP_ENV == 'development' ? '#!' : '') /* $c_or_p . '=' . (empty($_GET[$c_or_p]) ? '' : $$c_or_p->name) . '&amp;app=git' */ ?>"
                method="POST">
                <div
                  ...
                  <div class="text-sm" style="display: inline;">
                    <label id="gitStatusLabel" for="gitStatus"
                      style="background-color: hsl(343, 100%, 42%); color: white;">&#9650;
                      <code>Stage / Commit</code></label>
                  </div>
                  <div style="display: inline; float: right; text-align: center;">
                    <input id="gitStatusSubmit" class="btn" type="submit" value="Status/Run" />
                  </div>
                ...
                <div id="gitStatusForm"
                  ...
                  <label>Git Command</label>
                  <textarea cols="40" rows="6"
                    name="git[status]"><?= /* GIT_INIT_PARAMS  NULL*/ 'git commit'; ?></textarea>
                </div>
              </form>
            </div>

            <div id="app_git-frameUpdate" class="app_git-frame-container absolute"
              style="overflow: hidden; height: 270px;">
              <form autocomplete="off" spellcheck="false"
                action="?<?= http_build_query(APP_QUERY + ['app' => 'git']) . (APP_ENV == 'development' ? '#!' : '') /* $c_or_p . '=' . (empty($_GET[$c_or_p]) ? '' : $$c_or_p->name) . '&amp;app=git' */ ?>"
                method="POST">
                ...
                <div id="gitStatusForm"
                  ...
                  <label>Git Command</label>
                  <textarea cols="40" rows="6"
                    name="git[status]"><?= /* GIT_INIT_PARAMS  NULL*/ 'git update'; ?></textarea>
                </div>
              </form>
            </div>
      ...
      document.getElementById('requestInput').value = 'git push
      https://<?= getenv('COMPOSER_TOKEN') ?>@github.com/barrydit/codeinsync.git';
      ...
      */
      ...
      if (currentIndex <= 0) currentIndex=5; console.log(currentIndex + '!=' + git_frame_totalFrames); currentIndex--;
    ...
    git_frame_containers.eq(currentIndex).addClass(' selected'); // css("z-index", git_frame_totalFrames); // Set top
          layer z-index }); /* $("#app_git-push").click(function() { e.preventDefault(); });
          $("#app_git-pull").click(function() { }); $("#app_git-frameSelector").change(function() { var
          selectedIndex=parseInt($(this).val(), 10); currentIndex=selectedIndex; if (currentIndex>=
          git_frame_totalFrames) {
          currentIndex = 0;
          }
          ...
          if (currentIndex <= git_frame_totalFrames && currentIndex> 0) {
            $(".app_git-frame-container").eq(currentIndex).addClass("selected"); // Apply selected class to the chosen
            container
            }
            ...
            */
            ...
            //check if file is included or accessed directly
            if (cis_is_direct_http_file(__FILE__) && APP_DEBUG && ($_GET['app'] ?? null) === 'git')
              die($appGit['html']);
            ...
            */
            ...
            <div id="app_node_js-container"
              ...
              <div
                ...
                <div style="position: relative; overflow: hidden; width: 398px; height: 256px;">
                  ...
                  <div id="app_git-frameMenu" class="app_git-frame-container absolute selected"
                    style="background-color: rgb(225,196,151,.75); margin-top: 8px; height: 100%;">

                    <div
                      style="display: block; margin: 10px auto; width: 100%; background-color: rgb(255,255,255,.75);">

                      <div class="text-sm" style="display: inline-block; width: 100%;"><span>Dependencies
                          (install):</span>
                        <div style="position: relative; float: right;">
                          <input id="composerReqPkg" type="text" title="Enter Text and onSelect" list="composerReqPkgs"
                            placeholder="" value="" onselect="get_package(this);">
                  ...
                  <div id="app_git-frameConfig" class="app_git-frame-container absolute"
                    ...
                    <form autocomplete="off" spellcheck="false" action="?app=git#!" method="POST">
                      <div style="position: absolute; right: 0; float: right; text-align: center;">
                        <button id="gitConfigSubmit" class="btn absolute"
                          style="position: absolute; top: 0; right: 0; z-index: 2;" type="submit"
                          value="">Modify</button>
                      </div>
                      <div style="display: inline-block; width: 100%; background-color: rgb(225,196,151,.75);">
                        <div class="text-sm" style="display: inline;">
                      ...
                      <div id="appGitIgnoreForm"
                        style="display:inline-block; padding: 10px; background-color: rgb(235,216,186,.80); border: 1px dashed #0078D7;">
                        <label>Git Command</label>
                      ...
                      <div id="appGitConfigForm"
                        style="display: inline-block; overflow-x: hidden; overflow-y: auto; height: 180px; padding: 10px; background-color: rgb(235,216,186,.80); border: 1px dashed #0078D7;">
                        <div style="display: none; width: 100%;">
                          <input type="checkbox" name="gitConfigList">
                          <label style="font-style: italic;">git config -l</label>
                          <div style="float: right;">
            ...
            //check if file is included or accessed directly
            if (cis_is_direct_http_file(__FILE__) && APP_DEBUG && ($_GET['app'] ?? null) === 'npm')
              die($appNpm['html']);
...
//  }
            ...
            <div id="app_php-container" style="border: 1px solid #000; width: 400px;">
              <div class="header ui-widget-header">
                ...
                <div style="display: inline; float: right; text-align: center;">[<a
              ...
              <div class=" ui-widget-content" style="display: inline-block; width: auto; padding-left: 10px;">
                ...
                <form style="display: inline;"
                  action="<?= APP_URL_BASE . basename(APP_SELF) . '?' . http_build_query(APP_QUERY + ['app' => 'php']) . (APP_ENV == 'development' ? '#!' : '') /*  $c_or_p . '=' . (empty($_GET[$c_or_p]) ? '' : $$c_or_p->name) . '&amp;app=composer' */ ?>"
                  method="GET">
                    ...
                    <div style="display: inline; width: 46%;">
                      ...
                      <select name="const" onchange="this.form.submit()">
                        <?php foreach (get_defined_constants(true)['user'] as $key => $user_const) { ?>
                          <option <?= (isset($_GET['const']) && $_GET['const'] == $key ? ' selected' : '') ?>><?= $key; ?>
                          </option>
                        <?php } ?>
                      </select>
                    </div>
...
<?= /*$appPHP['script'];*/ NULL; ?>
            ...
            //check if file is included or accessed directly
            if (cis_is_direct_http_file(__FILE__) && APP_DEBUG && ($_GET['app'] ?? null) === 'php')
              die($appPHP['html']);
