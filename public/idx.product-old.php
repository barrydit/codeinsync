...
<head>
  ...
  <base
    href="<?= (!is_array(APP_URL) ? APP_URL : APP_URL_BASE) . (APP_URL['query'] != '' ? '?' . APP_URL['query'] : '') . (defined('APP_ENV') && APP_ENV == 'development' ? '#!' : ''); ?>">
  ...
  <div style="position: relative; width: 100%; height: 100%; z-index: 1; border: 1px solid green;">
    ...
    <div style="position: relative; margin: 0px auto; width: 100%; border: 1px solid #000;">
      ...
      <div style="position: relative; margin: 0px auto; width: 800px;">
        <div
          style="position: absolute; <?= /* (empty($errors) ? 'display: none;' : '') */ NULL; ?>left: -144px; /*width: 150px;*/ z-index: 3;">
          ...
          <input class="input" id="toggle-debug" type="checkbox" onchange="toggleSwitch(this); return null;"
            <?= (isset($_GET['debug']) || defined('APP_ENV') && APP_ENV == 'development' ? '' : '') ?> />
          <label class="label" for="toggle-debug" style="margin-left: -10px;">
            <div class="switch">
              <span class="slider round"></span>

              <div id="hide_notice-container"
                style="position: absolute; display: <?= (isset($errors['GIT_UPDATE']) ? 'block' : 'none') ?>; left: -20px; top: 40px; width: 100px; background-color: white; color: red; font-variant-caps: all-small-caps; text-align: center;">
                [<a onclick="getElementById('toggle-debug').click(); /*toggleSwitch(this);*/ return null;"
                  href="?hide=update-notice">Hide Notice</a>]</div>
            </div>

            <div class="right"
              style="background-color: #0078D7; display: <?= (isset($errors['GIT_UPDATE']) ? 'inline-block' : 'none') ?>; color: #FFF;">
              &nbsp;<span style="background-color: #FFF; color: #0078D7;">&quot;Update&quot;</span>&nbsp;</div>
          </label>
          <!-- /form -->
        </div>
        <div id="debug-content" class="absolute"
            ...
            <form style="display: inline;" action="/" method="POST">Stage:
              <select name="environment" onchange="this.form.submit();">
                <option value="develop" <?= (defined('APP_ENV') && APP_ENV == 'development' ? 'selected' : '') ?>>
                  Development</option>
                <option value="product" <?= (defined('APP_ENV') && APP_ENV == 'production' ? 'selected' : '') ?>>
                  Production</option>
          ...
          <div
            ...
            <div style="display: inline; margin-top: -7px; float: left; "><a
                style="font-size: 18pt; font-weight: bold; padding: 0 3px;" href="?path">&#8962; </a></div>
...
*/ ?>
      <div style="position: relative;">
        <?php if (isset($_GET['client']) && $_GET['client'] != '') { ?>
          <div id="app_client-container"
            style="position: relative; display: none; top: 100px; margin: 0 auto; width: 800px; height: 600px; background-color: rgba(255, 255, 255, 0.9); overflow-x: scroll;">
        ...
        <div id="app_directory-container"
          ...
          <?php } elseif (isset($_GET['application'])) { ?>
            ...
            <div style="display: inline-block; text-align: right; width: 300px; ">
              <form
                action="<?= APP_URL_BASE . '?' . http_build_query(APP_QUERY + ['app' => 'composer', 'path' => 'vendor']) ?>"
                method="POST">
                <input id="RequirePkg" type="text" title="Enter Text and onSelect" list="RequirePkgs"
                  placeholder="[vendor]/[package]" name="composer[package]" value="" onselect="get_package(this);"
                  autocomplete="off" style=" margin-top: 4px;">
  ...
  ?>
  <div
    style="position: fixed; display: none; top: 0; left: 0; z-index: 1; height: 500px; background-color: #FFF; width: 300px;">
...
-->
  ...
  <?php
  // (APP_IS_ONLINE && check_http_status('https://cdn.tailwindcss.com') ? 'https://cdn.tailwindcss.com' : APP_URL . 'assets/vendor/tailwindcss-3.3.5.js')? [[jquery-ui]-[1.12.1].js]
  is_dir($path = APP_BASE['public'] . 'assets/vendor/jquery/') or mkdir($path, 0755, true);
  ...
  ?>
  <script
    src="<?= (realpath(APP_PATH . ($path = APP_BASE['public'] . 'assets/vendor/jquery/3.7.1/jquery-ui-1.12.1.js')) ? $path : (check_http_status($url) ? substr($url, strpos($url, parse_url($url)['host']) + strlen(parse_url($url)['host'])) : '')) ?>"></script>
  ...
  <?php if (is_dir($path = APP_BASE['public'] . 'assets/vendor/ace')) { ?>
  ...
  <script type="text/javascript" charset="utf-8">
    ...
    */
    <?php
    if (is_dir($path = APP_BASE['public'] . 'assets/vendor/ace')) { ?>
