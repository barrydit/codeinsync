  ...
  <select name="environment" onchange="this.form.submit();">
    ...
    <option value="math" <?= defined('APP_ENV') && APP_ENV == 'math' ? 'selected' : '' ?>>Math</option>
      ...
      <div style="position: absolute; top: 40px; left: -15px; z-index: 1; background-color: white; border: <?= defined('APP_ROOT') && APP_ROOT != '' || isset($_GET['path']) ? '2px dashed red' : '1px solid #000'; ?>;">
        <div style="display: inline; margin-top: -7px; float: left; "><a style="font-size: 18pt; font-weight: bold; padding: 0 3px;" href="<?= isset($_GET['path']) ? '/' : '/?path' ?>">&#8962; </a></div>
...
if (date(/*Y-*/ 'm-d') == /*1928-*/ '08-07' ?? /*2023-*/ '03-30') { ?>
    ...
    <script src="<?= APP_BASE['resources'] . 'reels/disturbed_-_it_wasnt_me.js'; /*kassy frequency something_happened disturbed_-_it_wasnt_me.js adhd_song.js !empty($reels = glob(APP_PATH . 'resources/reels/*.js')) ? APP_BASE['resources'] . 'reels/' . basename(array_rand(array_flip(array_filter($reels, 'is_file')), 1)) : ''; APP_BASE['resources'] */?>" type="text/javascript" charset="utf-8"></script>
    <?php } ?>
